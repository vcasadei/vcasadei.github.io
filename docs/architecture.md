...
- An English post and its Portuguese translation are paired by giving both the same `translation_key` front-matter value. This pairing drives three things:
  ...
  - **The 404 page** (`_layouts/404.html`): GitHub Pages serves this one static page for every broken URL on the whole domain, so `page.lang` always resolves to the default (`en`) regardless of which URL 404'd — there's no build-time way to know the visitor was under `/pt/`. Instead, an inline script detects `window.location.pathname.indexOf('/pt/') === 0` after the page loads and swaps the heading and "Reload Game" label (sourced from `_data/locale.yml`) and the primary nav links (Archive → Arquivo, About → Sobre, pointing at their `/pt/...` URLs) to Portuguese. The embedded Space Invaders game itself is never translated.
