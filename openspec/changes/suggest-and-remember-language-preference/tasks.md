...
- [ ] 6.1 Push to a branch and open a PR; confirm the `build` GitHub Actions check passes
- [ ] 6.2 Download the built artifact and confirm `head/custom.html`'s output includes the redirect script and `header.html`'s output includes the banner markup on a sample of built pages
- [ ] 6.3 Confirm no unrelated pages regressed (diff a sample of untouched pages against a master build)
