...
## 5. Post-ship fixes (found via live user testing)

- [x] 5.1 Fix: the dropdown was clipped/unclickable in practice — `.navigation` has `overflow-x: auto; overflow-y: hidden` (from the theme's own `overflow` mixin, unrelated to this change), which clipped the menu since it was `position: absolute` inside that container. Switched to `position: fixed`, computed via `getBoundingClientRect()` on open, plus close-on-scroll since the header isn't sticky. Verified in a real build (PR #11) and live
- [x] 5.2 Fix: the header logo/site-title link always pointed at the English root (`/`), so clicking it from a Portuguese page sent visitors back to English. Made it language-aware (`/pt/` when `page.lang` isn't English). Verified in a real build (PR #12) and live
