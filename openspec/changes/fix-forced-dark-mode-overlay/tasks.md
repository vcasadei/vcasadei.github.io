...
- [x] 4.6 Ask the user to re-verify visually on the setup that reproduced this (macOS Chrome with OS appearance set to Light, and/or the WhatsApp in-app browser on Android) — this is the real confirmation the bug is gone, since grep only proves the CSS is absent, not that rendering looks correct to a human (confirmed by user: "checked, it works now")
